...
public interface AnimalRepository extends JpaRepository<Animal, Long> {
    @Query(value = "select animal.* from animal where animal.id=:idShelter and animal.id not in (SELECT animal_ownership.id_animal from animal_ownership);"
            , nativeQuery = true)
    List<Animal> getAllNotBusyAnimalsWithShelter(Long idShelter);

    @Query(value = "select animal.* from animal where animal.id=:idAnimal and animal.id_shelter=:idShelter and animal.id not in (SELECT animal_ownership.id_animal from animal_ownership);"
            , nativeQuery = true)
